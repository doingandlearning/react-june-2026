...
`defineConfig` comes from `vitest/config` here, not `vite` — Vite's own `defineConfig` doesn't know about the `test` field, so TypeScript can't match any overload and flags it as an error.
