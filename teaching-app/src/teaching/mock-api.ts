// Shared API simulation for Sessions 05–08
...
export function fetchTools(category?: string): Promise<Tool[]> {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve(category ? TOOLS.filter((t) => t.category === category) : TOOLS);
    }, 800);
  });
}
...
export function fetchToolById(id: string): Promise<Tool | null> {
  return new Promise((resolve) => {
    setTimeout(() => resolve(TOOLS.find((t) => t.id === id) ?? null), 400);
  });
}

export function submitTool(data: Omit<Tool, "id">): Promise<Tool> {
  return new Promise((resolve) => {
    setTimeout(() => {
      const tool = { ...data, id: String(Date.now()) };
      TOOLS.push(tool);
      resolve(tool);
    }, 600);
  });
